  ...
  if(var %in% c('tasmax','tasmin','hurs')){
    ...
    his_pth <- ifelse(var == 'tasmax',
                      ...
                      ifelse(var == 'tasmin',
                             ...
                             paste0('//catalogue/WFP_ClimateRiskPr1/1.Data/chirts_global/Rh')))
        ...
        purrr::map(.f = function(j){
          ...
          1:length(his_daily) |>
            purrr::map(.f = function(k){
              outfile <- paste0(fut_pth,'/',fut_daily[k]); dir.create(dirname(outfile),F,T)
...
map(.x = 1:length(gcms), .f = function(g){
  get_daily_future_data(gcm = gcms[g], ssp = ssp, var = 'hurs', prd = '2025_2055')
})
