...
## Models
mdls <- dir_ls('./rf/output/run_7_cocoa/models', regexp = '.rdata$')
...
vars <- readRDS('./rds/v5_cocoa/vars_cocoa.rds')
...
names(clma) <- gsub('bioc_20100', 'bioc_20_100', names(clma))
...
thrp <- readRDS(file = './rds/v5_cocoa/threshol_prob_cocoa.rds')
thru <- readRDS(file = './rds/v5_cocoa/threshol_uncr_cocoa.rds')

## AEZ
lbls <- tibble(value = 1:9, class = c('Unsuitable', 'Unsuitable', 
                                      'Constant - Very dry', 'Cold - Wet', 
                                      'Very hot - Wet', 'Hot - Very wet', 'Very cold - Very dry',
                                      'Limitations', 'Mixed'))
...
prdc <- reduce(prdc, c)
dir_ls('./rf/output/run_7_cocoa/results/process')
terra::writeRaster(x = prdc, filename = './rf/output/run_7_cocoa/results/process/rf-raw_ftr-gcms.tif', overwrite = TRUE)
...
terra::writeRaster(x = mixd, filename = './rf/output/run_7_cocoa/results/process/rf-mix_ftr-gcms.tif', overwrite = TRUE)
...
mixd <- map(.x = 1:nlyr(mixd), .f = function(i){
  levels(mixd)[[i]] <- data.frame(ID = 1:9, clase = c('Unsuitable', 'Unsuitable', 
                                                      'Constant – Very dry',
                                                      'Cold - Wet', 'Very hot - Wet', 
                                                      'Hot - Very wet', 'Very cold - Very dry', 
                                                      'Limitations', 'Mixed'))
  return(mixd[[i]])
})
mixd <- reduce(mixd, c)
...
names(clrs) <- c('Unsuitable', 'Constant – Very dry', 'Cold - Wet', 'Very hot - Wet', 
                 'Hot - Very wet', 'Very cold - Very dry', 'Limitations', 'Mixed')
...
ggsave(plot = ggps.all, filename = './png/maps/cocoa/rf_cluster5_cocoa_gcms.jpg', units = 'in', width = 12, height = 4, dpi = 300)
...
levels(mdal)[[1]] <- data.frame(
  ID = 1:9, 
  clase = c('Unsuitable', 'Unsuitable', 
            'Constant – Very dry',
            'Cold - Wet', 'Very hot - Wet', 
            'Hot - Very wet', 'Very cold - Very dry', 
            'Limitations', 'Mixed')
...
ggsave(plot = ggps.mdl, filename = './png/maps/cocoa/rf_cluster5_cocoa_gcms-modal.jpg', units = 'in', width = 12, height = 4, dpi = 300)
ggsave(plot = gmdl, filename = './png/maps/cocoa/rf_cluster5_cocoa_modal.jpg', units = 'in', width = 7, height = 3, dpi = 300)

## Join both 
ggall <- ggpubr::ggarrange(ggps.mdl, gmdl, ncol = 1, nrow = 2)
ggsave(plot = ggall, filename = './png/maps/cocoa/rf_cluster5_cocoa_modal_gcms.jpg', units = 'in', width = 15, height = 9.5, dpi = 300)

# To write the rasters ----------------------------------------------------
terra::writeRaster(x = mdal, filename = './rf/output/run_7_cocoa/results/process/rf-mix_ftr-modal.tif', overwrite = TRUE)
terra::writeRaster(x = mixd, filename = './rf/output/run_7_cocoa/results/process/rf-mix_ftr-gcms.tif', overwrite = TRUE)


# Join current and future map  --------------------------------------------

rasterRFclass <- rast('./rf/output/run_7_cocoa/results/process/rf-mixed_bsl.tif')
...
levels(rasterRFclass)[[1]] <- data.frame(ID = 1:9, clase = c('Unsuitable', 'Unsuitable', 'Arabica', 'Robusta', 'Limitations', 'Mixed'))
