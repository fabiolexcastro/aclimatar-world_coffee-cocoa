...
get_daily_future_data <- function(gcm, ssp, var, prd){
  ...
  if(var %in% c('pr','rsds')){
    ...
    if (var == 'pr') {
      ...
      his_str <- paste0('Solar-Radiation-Flux_C3S-glob-agric_AgERA5_',gsub(pattern='-', replacement='', x=mpg$Baseline, fixed=T),'_final-v1.1.nc')
      fut_str <- paste0('Solar-Radiation-Flux_C3S-glob-agric_AgERA5_',gsub(pattern='-', replacement='', x=mpg$Future, fixed=T),'_final-v1.1.tif')
    }
    # Split by months
...
map(.x = 1:length(gcms), .f = function(g){
  get_daily_future_data(gcm = gcms[g], ssp = ssp, var = 'rsds', prd = '2025_2055')
})
