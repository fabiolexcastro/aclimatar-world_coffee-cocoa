...
isos <- c("IND", "CAF", "GIN", "CIV", "MDG", "YEM", "PHL", "CMR")
...
# To classify -------------------------------------------------------------
stck <- indx.hist; iso <-  isos[1]; indx <- 'ndd'
clsf <- function(stck, iso, indx){
  ...
  vls <- terra::extract(rst, pnts[,1:2])[,2]
  qnt <- quantile(vls, c(0, 0.5, 0.75, 1), na.rm = T)
  
  ## To extract by mask
  rst <- terra::crop(rst, shp)
  rst <- terra::mask(rst, shp)
...
terra::writeRaster(x = indx.hist.cls, filename = './common_data/stack_final/stack_indices-hist_oth.tif', overwrite = TRUE)
terra::writeRaster(x = indx.ftre.cls, filename = './common_data/stack_final/stack_indices-ftre_oth.tif', overwrite = TRUE)
