...
## Parameters
gcm <- gcms[1]; ssp <- ssp; var <- 'tasmin'; prd <- '2025_2055'
        ...
        purrr::map(.f = function(j){
          ...
          1:length(his_daily) |>
            purrr::map(.f = function(k){
              outfile <- paste0(fut_pth,'/',fut_daily[k]); dir.create(dirname(outfile),F,T) # yrs_f_daily[k],'/',
...
map(.x = 1:length(gcms), .f = function(g){
  get_daily_future_data(gcm = gcms[g], ssp = ssp, var = 'tasmin', prd = '2025_2055')
})
